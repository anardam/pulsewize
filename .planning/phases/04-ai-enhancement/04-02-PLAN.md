...
Both files import their subject modules at the top level — module-not-found errors cause a true RED failure until 04-03/04-04 create the implementations.
  ...
  <verify>
    <automated>cd /Users/Mahesh.Sangawar/personal/anardam/SocialLens && npx vitest run src/lib/ai/orchestrator.test.ts src/app/api/analyze/route.test.ts --reporter=verbose 2>&1 | tail -20</automated>
  </verify>
  <acceptance_criteria>
...
```
  ...
  <verify>
    <automated>cd /Users/Mahesh.Sangawar/personal/anardam/SocialLens && npx vitest run src/lib/ai/compare.test.ts src/lib/ai/calendar.test.ts src/lib/ai/hashtags.test.ts --reporter=verbose 2>&1 | tail -30</automated>
  </verify>
  <acceptance_criteria>
