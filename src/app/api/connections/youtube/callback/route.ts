...
export async function GET(request: Request) {
  ...
  try {
    ...
    logInfo("youtube_connection_exchange_succeeded", {
      requestId,
      route: "youtube_callback",
      platform: "youtube",
      userId: user.id,
    });
    ...
    const upsertPayload = {
      ...
      display_name: identity.displayName ?? identity.username ?? "YouTube Channel",
    ...
    logError("youtube_connection_failed", error, {
      requestId,
      route: "youtube_callback",
      platform: "youtube",
      userId: user.id,
    ...
    cookieStore.delete(STATE_COOKIE);
    const message =
      error instanceof Error ? error.message : "youtube_failed";
    const redirectUrl = new URL("/settings", request.url);
    redirectUrl.searchParams.set("connection_error", "youtube_failed");
