...
import { buildCalendarPrompt } from "@/lib/prompt";
...
import type { ContentCalendarReport, ManualProfileInput } from "@/lib/types";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const prompt = buildCalendarPrompt(profile, platform);
    ...
    if (firstBrace === -1 || lastBrace === -1) {
      return NextResponse.json(
        { success: false, error: "Failed to parse calendar report" },
        { status: 500 }
      );
    }
    jsonStr = jsonStr.slice(firstBrace, lastBrace + 1);
    const report: ContentCalendarReport = JSON.parse(jsonStr);
